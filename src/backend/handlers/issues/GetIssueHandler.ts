...
export class GetIssueHandler extends BaseHandler {
  ...
  async handle(req: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
      ...
      const issue = await this.issueService.getIssueById(params.id, payload.id);
      return this.success(issue, 200, rateLimit);
    } catch (error: any) {
      return this.error(error.message || 'Failed to fetch issue', 400, rateLimit);
    }
  }
}
